// app.js（LocalStorage版：index.html一致・期間（複数）追加対応 + フィルタ操作ログ + 入力テンプレ）
...
function estimateSeverity(element, category, issueText) {
  ...
  if (category === "copyright" && (t.includes("配信") || t.includes("録画") || t.includes("既存曲") || t.includes("sns") || t.includes("ロゴ") || t.includes("素材"))) return "medium";
...
function extractIssues(text) {
  ...
  // 美術
  if (text.includes("美術") || text.includes("背景") || text.includes("写真") || text.includes("イラスト") || text.includes("素材") || t.includes("stock")) {
    add("美術", "copyright", "舞台美術や背景に写真/イラスト/素材を使う場合、出所（自作/購入/フリー）と利用範囲（上演/配信/SNS）を確認してください。");
  }

  // 衣装
  if (text.includes("衣装") || text.includes("ロゴ") || text.includes("ブランド") || text.includes("キャラ") || t.includes("costume")) {
    add("衣装", "copyright", "衣装にブランドロゴ・キャラクター等が入る場合、権利や商標・肖像の問題が出ることがあります。見え方（アップ/配信）を想定して対処してください。");
  }

  // SNS
  if (text.includes("sns") || text.includes("告知") || text.includes("投稿") || text.includes("拡散") || t.includes("instagram") || t.includes("x") || t.includes("twitter") || t.includes("tiktok")) {
    add("SNS", "copyright", "SNS告知画像/動画は素材（写真/フォント/ロゴ/BGM）の権利処理が必要です。投稿媒体の利用規約も確認してください。");
    add("SNS", "privacy", "SNSでの拡散は個人特定・誹謗中傷・炎上のリスクがあります。公開範囲や表現配慮を検討してください。");
  }
...
function generateQuestions(text) {
  ...
  if (t.includes("美術") || t.includes("写真") || t.includes("素材") || t.toLowerCase().includes("stock")) {
    push("美術/背景に使う素材は自作ですか？購入/フリー素材ですか？");
    push("配信やSNSにも掲載しますか？（利用範囲が広がる）");
  }
  if (t.includes("衣装") || t.includes("ロゴ") || t.includes("ブランド") || t.includes("キャラ")) {
    push("衣装のロゴ/キャラが映る可能性は？配信ではアップになりますか？");
    push("ロゴを隠す/差し替える等の対策は必要ですか？");
  }
  if (t.toLowerCase().includes("sns") || t.includes("告知") || t.includes("投稿")) {
    push("SNS告知画像/動画の素材（写真/フォント/BGM）の出所は明確ですか？");
    push("告知の投稿先（X/Instagram/TikTok等）の規約を確認しましたか？");
  }
  if (t.includes("未成年")) {
  ...
  if (!q.length) push("配信有無、素材の出所、改変範囲（脚本/演出）、SNS掲載の有無を追記できますか？");
...
function generateActionTemplates(issues) {
  ...
  issues.forEach((it) => {
    if (it.category === "copyright") {
      push("権利処理の表を作成（上演/配信/録画/SNS別：楽曲、音源、映像素材、台本、写真、美術素材、衣装）");
...
function generateTasksFromIssues(issues) {
  ...
  issues.forEach((it) => {
    ...
    if (it.category === "copyright" && it.element === "美術") {
      add("美術素材（写真/イラスト/背景）の出所とライセンスを整理");
      add("配信/SNSでの掲載可否を確認");
    }
    if (it.category === "copyright" && it.element === "衣装") {
      add("衣装のロゴ/キャラが映る箇所を確認し対策案を決める");
    }
    if (it.element === "SNS") {
      add("SNS告知素材（画像/動画/フォント/BGM）の出所を整理");
      add("投稿先の利用規約（素材/BGM/二次利用）を確認");
    }
    if (it.category === "privacy") {
      add("撮影範囲と掲示文を確定");
  ...
  if (!tasks.length) add("不足情報の確認（配信/素材出所/改変範囲/SNS）");
  return tasks;
}
...
function buildTemplateText() {
  ...
  if (tpl("t_stream")?.checked) lines.push("・配信あり（ライブ）");
  if (tpl("t_archive")?.checked) lines.push("・アーカイブ公開あり");

  if (tpl("t_existing_music")?.checked) lines.push("・既存曲を使用（BGM/歌）");
  if (tpl("t_recorded_music")?.checked) lines.push("・録音音源を使用（生演奏ではない）");

  if (tpl("t_art")?.checked) lines.push("・美術素材を使用（写真/イラスト/背景など）");
  if (tpl("t_costume")?.checked) lines.push("・衣装に既製品/ロゴ/キャラクター要素がある");

  if (tpl("t_sns")?.checked) lines.push("・SNS告知（画像/動画）あり");
  if (tpl("t_minors")?.checked) lines.push("・未成年出演あり");

  if (tpl("t_backstage")?.checked) lines.push("・舞台裏/楽屋が映る可能性あり");
  if (tpl("t_strobe")?.checked) lines.push("・ストロボ/点滅照明あり");

  if (tpl("t_real_event")?.checked) lines.push("・実在事件/災害が題材");

  const extra = (tpl("t_extra")?.value || "").trim();
...
btnResetTemplate?.addEventListener("click", () => {
  [
    ...
    "t_art","t_costume","t_sns","t_minors","t_backstage","t_strobe","t_real_event"
  ].forEach(id => {
    const el = tpl(id);
    if (el) el.checked = false;
  });
  const ex = tpl("t_extra");
  if (ex) ex.value = "";
});
